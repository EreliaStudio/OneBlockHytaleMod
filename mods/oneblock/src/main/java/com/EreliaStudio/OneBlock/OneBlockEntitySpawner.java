...
public final class OneBlockEntitySpawner
{
    ...
    public static boolean spawnNpc(Store<EntityStore> store, World world, Vector3i basePos, String entityId)
    {
        ...
        if (!configureInteraction(interaction, entityId)) return false;

        world.execute(() ->
        {
            try { methodSpawnNpc.invoke(interaction, store, basePos); }
            catch (Exception e) { LOGGER.at(Level.SEVERE).log("Failed to spawn NPC '" + entityId + "': " + e.getMessage()); }
        });
        return true;
    }
    ...
    {
        if (initialized) return fieldEntityId != null && methodSpawnNpc != null;
