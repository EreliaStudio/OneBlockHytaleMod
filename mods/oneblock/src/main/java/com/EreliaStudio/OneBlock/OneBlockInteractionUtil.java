...
final class OneBlockInteractionUtil
{
    ...
    {
        if (interactionContext == null || heldItem == null) return;
        ...
        ItemStack updatedStack = (quantity <= 1) ? ItemStack.EMPTY : heldItem.withQuantity(quantity - 1);

        if (container != null && slot >= 0)
        {
            if (updatedStack.isEmpty()) container.removeItemStackFromSlot(slot);
            else container.setItemStackForSlot(slot, updatedStack);
        }
    ...
    static void skip(InteractionContext interactionContext)   { setState(interactionContext, InteractionState.Skip); }
    ...
    static void fail(InteractionContext interactionContext, HytaleLogger logger, String reason)
    {
        ...
        if (logger != null && reason != null && !reason.isEmpty()) logger.at(Level.WARNING).log(reason);
    }
